...
class SaveForegroundService : Service() {
  ...
  override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
    when (intent?.action) {
      ACTION_START, ACTION_UPDATE -> {
        ...
        showProgress(done = done, total = total, title = title, body = body, alert = intent.action == ACTION_START)
      }
      ACTION_COMPLETE -> {
        val title = intent.getStringExtra(EXTRA_TITLE)
        val body = intent.getStringExtra(EXTRA_BODY)
  ...
  private fun showProgress(done: Int, total: Int, title: String?, body: String?, alert: Boolean) {
    ...
    // Combine the progress (e.g., "0/9") with the custom body text if provided.
    val progressText = if (safeTotal > 0) "${safeDone.coerceAtMost(safeTotal)}/$safeTotal" else "Working"
    val fullBodyText = if (body.isNullOrBlank()) progressText else "$progressText • $body"
    ...
    val builder = NotificationCompat.Builder(this, channelId)
      ...
      .setContentText(fullBodyText)
      .setStyle(NotificationCompat.BigTextStyle().bigText(fullBodyText))
  ...
  private fun showCompleted(title: String?, body: String?) {
    ...
    ensureChannel(channelId, "Completed", NotificationManager.IMPORTANCE_HIGH)
  ...
  companion object {
    ...
    private const val CHANNEL_COMPLETE = "save_complete_v2"
